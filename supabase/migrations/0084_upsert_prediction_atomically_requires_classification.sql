-- Migration: 0084_upsert_prediction_atomically_requires_classification
-- Persistent Metagame Phase 1.
--
-- 0063's own upsert_prediction_atomically is not edited as a file —
-- this follows the exact drop-then-recreate precedent already
-- established in this repository (0049, 0078, and others). The only
-- behavioral change: a Match must have a declared
-- activity_classification (0082/0083) before it will accept a first
-- Prediction. Every other check/lock/upsert behavior is byte-for-byte
-- unchanged from 0063.
--
-- This is the enforcement half of "classification must be declared
-- before a Prediction can be accepted" — 0083 enforces immutability
-- once evidence exists; this enforces the precondition on the other
