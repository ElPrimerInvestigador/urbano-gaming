-- Migration: 0037_start_session_atomically_accepts_segment_target
-- Slice 008 — Segment / Turn grouping.
--
-- START_SESSION gains one new parameter, p_segment_target, defaulting
-- to 'NEW_SEGMENT' so every pre-Slice-008 caller (including any client
-- still running cached JS that never sends this field at all) keeps
-- working byte-for-byte unchanged.
--
--   'NEW_SEGMENT'     -> allocate the next segment_ordinal for this
--                         session and create a new Segment before
--                         creating the Interaction Instance (a new
--                         member-facing Turn).
--   'CURRENT_SEGMENT' -> create no new Segment; attach the new
--                         Interaction Instance to the session's
--                         existing current Segment instead (the
--                         mechanism behind the Best Joke proving case —
--                         Open Response, then Voting, same Turn).
--
-- Every existing precondition (previous interaction instance, if any,
-- must be RESULT_REVEAL) applies identically to both targets — this
-- migration does not relax it for CURRENT_SEGMENT.
--
-- Ordinal allocation safety: this function's first statement (line
-- ~50 below, unchanged from 0033) already locks the parent sessions row
-- with `for update`, held for the function's entire duration — a
-- concurrency review across three founder-directed passes traced every
-- START_SESSION race against this exact lock and confirmed it fully
-- serializes the function per session, independent of segment_target.
-- `select coalesce(max(segment_ordinal), 0) + 1 from segments where
-- session_id = p_session_id` is therefore safe to compute as a plain
-- read here — it is safe SPECIFICALLY because it executes after that
-- lock is already held, not because MAX+1 is inherently race-free. The
-- UNIQUE (session_id, segment_ordinal) constraint on segments (0035)
-- remains the actual final integrity guarantee regardless.
--
-- Standing dependency this design relies on: every code path capable of
-- creating a Segment must first acquire this same parent Session-row
-- lock. If a future slice introduces a second way to create a Segment
-- without it, this allocation strategy must be revisited.
--
-- created_at plays no role in Segment ordering here or anywhere else —
-- segment_ordinal is allocated directly, under the lock, never derived
-- from a timestamp.
--
-- Signature change (7 args -> 8): requires the drop-then-create pattern
-- established in 0017-0020 and reused in 0022, 0026, and 0033.
-- Return shape gains segment_ordinal alongside the existing four
-- columns.

drop function if exists start_session_atomically(uuid, text, text, uuid, text, jsonb, uuid);
...
begin
  ...
  -- see this migration's header comment for why that makes the
  -- MAX+1 allocation below safe.
