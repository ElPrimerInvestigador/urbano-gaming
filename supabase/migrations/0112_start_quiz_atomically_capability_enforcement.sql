-- Migration: 0112_start_quiz_atomically_capability_enforcement
-- Session Capability Architecture v1.
--
-- 0042's own start_quiz_atomically is not edited as a file —
-- drop-then-recreate, the same precedent as every prior replacement.
-- The only behavioral change: one new early guard, placed immediately
-- after the existing LOBBY_NOT_LOCKED check, requiring the QUIZ
-- capability before this Session may start a Quiz.
--
-- This function is start_session_atomically's (0111) structurally
-- separate sibling — Quiz's own Multiple Choice Interaction Instances
-- are created exclusively here, never through start_session_atomically
-- — so QUIZ and TRIVIA are enforced at two independent call sites,
-- never by inspecting a resulting Interaction Instance's own
-- engine_type after the fact. See Session_Capability_Architecture.md's
-- "Quiz and Trivia Are Distinct Capabilities" section.
--
-- Everything else is byte-for-byte unchanged from 0042.
