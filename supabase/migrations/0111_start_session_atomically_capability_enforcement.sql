-- Migration: 0111_start_session_atomically_capability_enforcement
-- Session Capability Architecture v1.
--
-- 0039's own start_session_atomically is not edited as a file —
-- drop-then-recreate, the same precedent this function's own history
-- already established. The only behavioral change: one new early
-- guard, placed immediately after the existing LOBBY_NOT_LOCKED check,
-- rejecting activation of an engine this Session has not declared.
--
-- Product-capability mapping (Session_Capability_Architecture.md,
-- ADR-036) — deliberately NOT keyed on engine_type alone, since
-- MULTIPLE_CHOICE is shared by two distinct Product capabilities:
--
--   p_prepared_question_id supplied  -> engine MULTIPLE_CHOICE -> TRIVIA
--   p_voting_source_type   supplied  -> engine VOTING           -> VOTING
--   neither supplied                 -> engine OPEN_RESPONSE    -> OPEN_RESPONSE
--
-- QUIZ's own MULTIPLE_CHOICE Interaction Instances are created
-- exclusively by start_quiz_atomically (0112), a structurally separate
-- function this one never calls into and never shares a code path
-- with — so this guard correctly requires TRIVIA for every
-- MULTIPLE_CHOICE activation that reaches this function, with no risk
-- of misclassifying a QUIZ-originated instance, because no
-- QUIZ-originated instance is ever created here.
--
-- Everything else is byte-for-byte unchanged from 0039.
