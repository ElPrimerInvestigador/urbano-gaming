-- Migration: 0104_record_experience_summary_atomically_xp_eligibility
-- Soccer Predictions — XP Eligibility / Calibration Support Slice.
-- 0097's own record_experience_summary_atomically is not edited as a
-- file — drop-then-recreate, same precedent as every prior
-- replacement. The only behavioral change: one new trailing
-- parameter, p_xp_eligible boolean not null default false, accepted
-- and stored, unvalidated here beyond its own NOT NULL/default —
-- this function remains generic and Experience-agnostic; it has no
-- business deciding what "eligible" means for any particular
-- Experience, only storing the fact it is handed.
--
-- default false (not true) is deliberate and matches 0103's own
-- column default exactly: a caller that omits this parameter — any
-- test/fixture seam, or a future Experience adapter not yet updated —
-- produces a non-XP-eligible Summary, never a silently XP-eligible
-- one. Predictions' own finalize/correct functions (0106/0107) always
-- pass this explicitly, computed from the governing Match's own
-- locked xp_eligible fact; they never rely on this default.

drop function if exists record_experience_summary_atomically(uuid, text, text, text, text, timestamptz, timestamptz, boolean, text, text, text, uuid, text, jsonb, integer, text[]);
...
create function record_experience_summary_atomically(
  ...
  p_xp_eligible boolean default false
)
returns table (
...
begin
  ...
  insert into experience_summaries (
    ...
    correct_dimension_count, correct_dimension_keys, xp_eligible
  )
  values (
    ...
    p_correct_dimension_count, p_correct_dimension_keys, coalesce(p_xp_eligible, false)
  )
