-- Migration: 0099_correct_match_result_atomically_predictions_v2
-- Soccer Predictions v2. 0092's own correct_match_result_atomically is
-- not edited as a file — drop-then-recreate, same precedent as every
-- prior replacement of this function. Applies the identical set of
-- changes documented in 0098 to the correction path, since this
-- function independently inlines its own copy of the same evaluation
-- logic (no shared SQL helper exists between the two functions in
-- this codebase's own convention) — see 0098's own comment for the
-- full rationale behind each change; not restated in full here.
--
-- One correction-specific consequence worth naming explicitly: with
-- the Goal-Minute fix in place, a correction that changes an official
-- goal from, say, ordinary minute 46 to 45+1 is now visible to
-- settlement — the original (46, null) and corrected (45, 1) are
-- structurally distinct tuples, so a Prediction fixed at ordinary 46
-- correctly evaluates as correct against the original Result Version
-- and incorrect against the corrected one. Under the prior flattened
-- scheme this correction would have been invisible to settlement
-- (both summed to 46). This function's own reversal/reissue mechanics
-- (via process_experience_summary_consequences_atomically) are
-- otherwise unchanged and require no further edit to handle this
-- correctly — the fix is entirely in what facts get reported, not in
-- how corrections are processed.
--
-- The cancelled/abandoned Match guard applies identically here: if
-- the Match has since been cancelled, a correction may not produce a
-- settlement either, for the same reason finalization may not.
