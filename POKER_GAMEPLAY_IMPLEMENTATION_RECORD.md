...
~~Local only. Migrations `0072`–`0081` have never been applied to production (production's migration ceiling remains `0044`, confirmed via the same authoritative `supabase migration list --linked` check used throughout this engagement). No push, no deploy, no SMTP.~~ **Superseded 2026-08-20.**

## Production Deployment & Validation (2026-08-20)

Migrations `0045`–`0081` applied to production in exact numeric order via `supabase db push --linked`; ledger confirmed at exactly `0081` via `supabase migration list --linked`; schema-only `supabase db dump --linked` confirmed all 21 new tables present with RLS auto-enabled and zero policies, zero drift from the pre-migration 13-table baseline otherwise. Commit `f030558` fast-forward pushed to `origin/main` (`0d38b0f..f030558`); GitHub → Vercel deployment confirmed live at `https://urbano-gaming-playtest.vercel.app` (new routes — `poker-host.html`, `poker-table.html`, `/api/gaming/poker/tables` — returning 200 where they previously 404'd).

A real production proving case was run on a live table (Host + Alex + Jordan + Sam), using the production database and the deployed application's own API throughout:

- **Table foundation**: table created (`startingStack:200, smallBlind:5, bigBlind:10`), 3 Guests joined and seated correctly; each participant's `myHoleCards` contained only their own two cards; the Host's own view had `myHoleCards: null` and every seat's `revealedHoleCards: null` pre-Hand; `deckOrder` confirmed absent from every response payload throughout the entire proving case.
- **Hand A (normal game)**: blinds posted correctly (5/10), full Pre-flop → Flop → Turn → River progression via real Check/Call actions with automatic street advancement (no host action required), Showdown reached automatically, correct winner determined by kicker (Alex's Two Pair with Queen kicker beat two other Two-Pair hands), payout applied atomically. Chip total before/after: 600/600.
- **Hand B (all-in / side pot)**: Jordan and Sam went all-in for 190 each, Alex (larger stack) went all-in for 220, triggering automatic runout straight to Showdown (no further betting decisions existed). Settlement produced a correctly decomposed **main pot of 570** (all 3 eligible, won by Jordan's Ace-kicker) and a **side pot of 30** (only Alex eligible, automatically returned to him — the degenerate single-contributor case, handled by the same general algorithm rather than a special case). Chip total before/after: 600/600.
- **Hand C (early fold win)**: heads-up (Sam correctly excluded — zero stack); Alex folded pre-flop; Jordan won immediately with `board: []` and `showdownHands: null` — no future-card leak, no unnecessary reveal. Chip total conserved.
- **Next Hand / idempotency**: dealer rotated correctly across all 5 Hands played; a double `POST /hand` call returned the identical `pokerHandId` with `alreadyStarted: true` on the second call (no duplicate Hand created); a duplicate `PLAYER_ACTION` replayed with the same idempotency key returned `alreadyApplied: true` with no double chip deduction (verified directly against `committedThisHand`); a mid-Hand joiner (added via the live mobile client) was correctly excluded from the Hand in progress and correctly included in the next one.
- **Reconnect**: every state check throughout this proving case was a fresh, independent `GET` using only a token — no client-side memory was ever relied on, which is the reconnect guarantee by construction.
- **Mobile (375×812, real production)**: join screen, waiting-seat view, active-turn view (hole cards, board, pot, stack, legal-action buttons, amount field, "YOUR TURN" indicator), post-action view, and Hand-result view all captured via real screenshots against the live production URL — no horizontal overflow, all legible. One `computer` click-dispatch timeout occurred (the same previously-documented Browser-pane tooling artifact from local development) and was worked around by dispatching the real DOM click via JavaScript rather than substituting a raw API call — the actual UI code path was still exercised.

**A pre-push production incident was investigated and closed as a test-authoring artifact, not a Product, migration, or deployment defect.** Initial smoke testing of the still-deployed pre-Identity/pre-Poker application (both before and immediately after the `f030558` deployment) showed reproducible `500` errors on `GET /api/sessions/[id]`, `/lock`, and `/start`. Direct database checks throughout (raw RPC calls, raw PostgREST table reads) succeeded, proving the schema, RLS, and RPCs were healthy the entire time. Root cause: `/lock` and `/start` take the session's **UUID** as their `[identifier]` path segment (per that route's own code comment), unlike `/join`, which takes the room code — the diagnostic script had passed the room code to all of them. Retesting with the correct identifier succeeded immediately and consistently, both before and after the deployment. No code or migration change was made or needed.

Identity Foundation (0045–0049) and Soccer Predictions (0050–0066) migrations were applied in the same batch but are documented separately in their own records. This record does not claim Poker complete beyond the v1 rules actually implemented — no tournament play, no rebuy, no disconnect/AFK handling, no Gaming XP integration, no rating.
